...
public class NullableDecimalCodecTest extends OpenFastTestCase {
    ...
    public void testBoundaries() {
        ...
        assertEquals("00000000 11000000 00000000 01111111 01111111 01111111 01111111 01111111 01111111 01111111 01111111 11111111", encode(new Decimal(Long.MAX_VALUE, 63)));
        assertEquals("11000001 01111111 00000000 00000000 00000000 00000000 00000000 00000000 00000000 00000000 10000000", encode(new Decimal(Long.MIN_VALUE, -63)));
    }

    public void testEncodeLargeDecimalReportsError() {
        try {
            basic.encode(buffer, new Decimal(150, 64));
            fail();
    ...
    public void testEncode() {
        assertEquals("10000011 10000001"                           , encode(new Decimal(1, 2)));
        assertEquals("11111101 00001011 10101101"                  , encode(new Decimal(1453, -3)));
        assertEquals("10100100 10000101"                           , encode(new Decimal(5, 35)));
        assertEquals("10000011 00111001 01000101 10100011"         , encode(new Decimal(942755, 2)));
        assertEquals("11111110 00111001 01000101 10100011"         , encode(new Decimal(942755, -2)));
        assertEquals("10000001 10000100"                           , encode(new Decimal(4, 0)));
        assertEquals("10000011 10000100"                           , encode(new Decimal(4, 2)));
        assertEquals("11111111 10000100"                           , encode(new Decimal(4, -1)));
        assertEquals("10000100 10000001"                           , encode(new Decimal(1, 3)));
        assertEquals("10000010 00000100 00111111 00110100 11011110", encode(new Decimal(9427550, 1)));
    }
    private ByteBuffer encode(Decimal decimal) {
        buffer.clear();
        basic.encode(buffer, decimal);
        return buffer;
    }
